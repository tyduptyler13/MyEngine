...
public:
	...
	std::shared_ptr<Vector3<T>> intersectSphere(const Sphere<T>& sphere,
		...
		if (t0 < 0) return optionalTarget = std::make_shared<Vector3<T>>(at(t1));

		return optionalTarget = std::make_shared<Vector3<T>>(at(t0));
	...
	std::shared_ptr<Vector3<T>> intersectPlane(const Plane <T>& plane, std::shared_ptr<Vector3<T>> optionalTarget) const {
		...
		return optionalTarget = std::make_shared<Vector3<T>>(at(t));
