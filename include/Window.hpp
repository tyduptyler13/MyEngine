...
namespace MyEngine {
	...
	/**
	 * A combinable bit mask signifying which mouse buttons are pressed
	 *
	 * Use a bit filter to check which of these is pressed at the same time.
	 */
	...
	typedef std::function<void(MouseButton button, EventType type,
														 unsigned x, unsigned y,
	                           unsigned dx, unsigned dy)> MouseEventHandler;
	typedef std::function<void(KeyboardKey key, EventType type)> KeyboardEventHandler;
	// Missing scrolling
	...
	struct Window {
		...
		/**
		 * Inform the system compositor that the frame is ready to be swapped.
		 *
		 * Defaults to an empty method in case the system is not double buffered on the window context.
		 */
		virtual Window* swapBuffers() {
			return this;
		};
